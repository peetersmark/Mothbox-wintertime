...
def main():
    ...
    for i in range(1, loop_iterations + 1):
        ...
        if mean_b is None:
            ...
            if diff_pct <= tolerance_pct:
                ...
                break
            # Early stop: reached MinExposure and still too bright -> reuse current image
            if current_exposure <= min_exposure and mean_b > target_mean:
                ...
                reuse_iteration_stage = f'iter{i}'
                break
            # proportional multiplicative update with gamma correction
            # Dynamic exponent based on normalized error, capped by GammaExponent from CSV
            error = target_mean - mean_b
            norm_error = error / target_mean if target_mean > 0 else 0.0
            abs_norm = abs(norm_error)
            # Scale exponent from 1.0 (near target) up to gamma (far), error scale ~0.6
            scale = abs_norm / 0.6
            if scale < 0:
                scale = 0
            if scale > 1:
            ...
            # Overshoot damping: when error sign flips, limit change magnitude for a couple iterations
    ...
    # Append brightness value to final filename
    final_mean_label = None
    final_name = None
    # If we already plan to reuse, we know the mean
    if reuse_final and reused_mean is not None:
        final_mean_label = int(round(reused_mean))
    # Construct initial final_name; may adjust after computing mean for non-reuse path
    ...
    if reuse_final and reused_file and reused_file.is_file():
        ...
        if final_res and final_res.get('returncode') == 0 and final_name.is_file():
            ...
            # If no brightness label yet, rename file to include mean
            if final_mean_label is None and mean_b_final is not None:
                final_mean_label = int(round(mean_b_final))
                new_name = Path(args.out_dir) / f"rpicam_{final_ts}_ex{final_exposure}us_mean{final_mean_label}.jpg"
                try:
                    final_name.rename(new_name)
                    final_name = new_name
                except Exception:
                    pass
